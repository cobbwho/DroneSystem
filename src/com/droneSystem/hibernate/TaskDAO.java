...
public class TaskDAO extends BaseHibernateDAO {
	...
	public Task findById(java.lang.Integer id) {
		...
		try {
			Task instance = (Task) getSession().get(
					"com.droneSystem.hibernate.Task", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Task instance) {
		...
		try {
			List results = getSession().createCriteria(
					"com.droneSystem.hibernate.Task").add(
	...
	public List findByHighway(Object highway) {
		return findByProperty(HIGHWAY, highway);
	}
	...
	public Task merge(Task detachedInstance) {
		...
		try {
			Task result = (Task) getSession().merge(
					detachedInstance);
