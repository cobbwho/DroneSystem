...
(function ($) {
    ...
    function _1bb(_1bc) {
        ...
        var win = $(_1bc).panel($.extend({}, _1bd.options, { border: false, doSize: true, closed: true, cls: "window", headerCls: "window-header", bodyCls: "window-body " + (_1bd.options.noheader ? "window-body-noheader" : ""), onBeforeDestroy: function () {
            ...
            //alert('begin by zhan');
			var newLeft = $(document).scrollLeft() + ($(window).width()-_1bd.window.outerWidth())*0.5;
			var newTop = $(document).scrollTop() + ($(window).height()-_1bd.window.outerHeight())*0.5;
			if(newLeft < 0){
				newLeft = 0;
			}
			if(newTop < 0){
			...
			_1b7(_1bc, {left:newLeft, top:newTop});	//移动窗体至中央位置
			//alert(end by zhan); //改动的地方还包括下面阴影部分的left和top为newLeft和newTop
			
			if (_1bd.mask) {
                //_1bd.mask.css({ display: "block", zIndex: $.fn.window.defaults.zIndex++ });
				
				//alert begin:by zhan(阴影图层区域)
				var maxWidth = $(document).width();
				var maxHeight = $(document).height();
				if(maxWidth < $(document).scrollLeft()+$(window).width()){
					maxWidth = $(document).scrollLeft()+$(window).width()
				}
				if(maxHeight < $(document).scrollTop()+$(window).height()){
					maxHeight = $(document).scrollTop()+$(window).height()
				}
				_1bd.mask.css({ display: "block", zIndex: $.fn.window.defaults.zIndex++, left: 0, top: 0, width:maxWidth, height:maxHeight });
				//end:by zhan
            }
            if (_1bd.shadow) {
                _1bd.shadow.css({ display: "block", zIndex: $.fn.window.defaults.zIndex++, left: newLeft, top: newTop, width: _1bd.window.outerWidth(), height: _1bd.window.outerHeight() });
            }
...
(function ($) {
    ...
    }, length: { validator: function (_331, _332) {
        var len = $.trim(_331).length;
        return len >= _332[0] && len <= _332[1];
    }, message: "Please enter a value between {0} and {1}."
    }, remote: { validator: function (_333, _334) {
        var data = {};
        data[_334[1]] = _333;
...
(function ($) {
    ...
    $.fn.form.methods = { submit: function (jq, _34a) {
        return jq.each(function () {
			//by zhan begin:添加一个属性，用于后台区分该请求是由form('submit')发出的
			var objTemp = $(this).find("input[name='form_submit_request_type_byzhan']");
			if(objTemp.length == 0){
				var form = $(this);
				var newInput = document.createElement("input");
				newInput.type="hidden";   
				newInput.name="form_submit_request_type_byzhan"; 
				newInput.value="form_submit_ajax";
				  
				form[0].appendChild(newInput); 
			}else{
				objTemp.each(function(){
					$(this).attr("value", "form_submit_ajax");
				});
			}
			//by zhan end
...
(function ($) {
    ...
    function _40b(_40c) {
        ...
        _40e.find("div.datagrid-cell").unbind(".datagrid").bind("click.datagrid", function () {
            ...
            if (opts.remoteSort) {
				//_4ae(_40c);  by zhan
				var data = $.data(_40c, "datagrid").data;	//by zhan
				_43dzhan(_40c, data);  //by zhan (均用前台排序)  
            } else {
	...
	function _43dzhan(_43e, data) {	//by zhan 加载数据（同_43d的功能）时前台排序
        var opts = $.data(_43e, "datagrid").options;
        var wrap = $.data(_43e, "datagrid").panel;
        var _43f = $.data(_43e, "datagrid").selectedRows;
        data = opts.loadFilter.call(_43e, data);
...
(function ($) {
    ...
    function _6c0(_6c1, data, _6c2) {
		...
		//	alert("by zhan");
            if (_6c4.length) {
       //         _6b5(_6c1, [_6c4[_6c4.length - 1]], _6c2);	//delete by zhan(数据加载完成不设置Value,避免吃字的情况)
            } else {
       //         _6b5(_6c1, [], _6c2);      //delete by zhan(数据加载完成不设置Value,避免吃字的情况)
            }
        }
